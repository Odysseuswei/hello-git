...
int main(int argc, char **argv) {
  ...
  sprintf(filename,"sde_adaptive_lorenz_COM_mali.txt");
  fp = fopen(filename,"w");
...
//    float Eps[] = {  0.05, 0.01 , 0.0};
  float Eps[] = { 0.1, 0.05, 0.02, 0.01, 0.0};
...
void sde_adaptive_l(int l, int N, double *sums) {
	...
	double T=12.0, X0[D], sum1=0.0, sum2=0.0, sum3=0.0, sum4=0.0, sum5=0.0, sum6=0.0, sum7=0.0,sum8=0.0;
  X0[0] = -2.4;
  X0[1] = -3.7;
  X0[2] = 14.98;
	      ...
	      //    int offset = n1*M; // number of random numbers already used
	       ...
	       //   int ns = 0;
	          ...
	          if (l==0) {
	              	...
	              	while(t<T){
                        if(ns>(RV_NUM-2*D-1)){
	                      ...
	                      if(tf==t){
                            ...
                            //    Pf += Xf[i+3]*dWf[i];
                            //}
                            Pf += Xf[0]*(dWf[1]+lamdaf[1]*hf/sigmaf);
                            //Pf += Xf[0]*hf; //Asian
	                    	    for(int i=0; i<D; i++){
	                      ...
	                      if(tc==t){
                            ...
                            //    Pc += Xc[i+3]*dWc[i];
                            //}
                            Pc += Xc[0]*(dWc[1]+lamdac[1]*hc/sigmac);
                            //Pc += Xc[0]*hc; //Asian
	                    	    for(int i=0; i<D; i++){
               ...
               Pf = Pf/sigmaf*Xf[2]*exp(Cmf[0]+Cmf[1]+Cmf[2]);


	             // std::cout<<Pf<<"\n";
	             if(l>0){
	                 // Pc = (Pc*S/sigmac*Xc[2]+Xc[5])*exp(Cmc[0]+Cmc[1]+Cmc[2]);
                  Pc = Pc/sigmac*Xc[2]*exp(Cmc[0]+Cmc[1]+Cmc[2]);
                  // Pc = 0;
                   dP = Pf-Pc;
	             }
	             else{
...
//  driftf[4] = X[0]+ (rho-X[2])*X[3]- (10+S)*X[4] -X[0]*X[5];
